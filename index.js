...
const { audio1, audio2 } = require('./config.json');
...
client1.on(Events.InteractionCreate, async interaction => {
	...
	try {
		connections = await command.execute(interaction, client1, client2, connections,message);
...
client1.login(audio1.TOKEN);
client2.login(audio2.TOKEN);
